...
//!
//! `legacy_ble_bt.rs` is the copied source baseline.  It is deliberately not
//! compiled: it contains the retired command/GATT payload dispatcher.  This
//! crate owns only the native callback boundary while Main wires opaque CoC
//! bytes into its shared QUIC-lite connection service.
...
#[no_mangle]
pub unsafe extern "C" fn dmesh_nimble_on_subscribe(_attr_handle: c_ushort, _notify: c_uchar) {}

#[no_mangle]
pub unsafe extern "C" fn dmesh_nimble_on_write(_data: *const c_uchar, _len: c_ushort) {}

#[no_mangle]
pub unsafe extern "C" fn dmesh_nimble_on_scan(
    _service_data: *const c_uchar,
    _len: c_ushort,
    _rssi: i8,
) {
}
