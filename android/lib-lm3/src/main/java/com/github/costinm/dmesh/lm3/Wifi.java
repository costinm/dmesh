    ...
    public void con2(Bundle msg, int mode) {
        if (mode == -1 && Build.VERSION.SDK_INT >= 29) {
            ...
            WifiP2pConfig cfg = new WifiP2pConfig.Builder()
                    ...
                    .build();
            if (ActivityCompat.checkSelfPermission(ctx, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(ctx, Manifest.permission.NEARBY_WIFI_DEVICES) != PackageManager.PERMISSION_GRANTED) {
