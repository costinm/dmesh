...
public class Hex {
     ...
     * is {@code true} odd-dataLength inputs are allowed and the first character is interpreted
    ...
    public static byte[] decode(char[] encoded, boolean allowSingleChar) throws IllegalArgumentException {
        ...
        if (allowSingleChar) {
            ...
            if ((encoded.length % 2) != 0) {
                throw new IllegalArgumentException("Invalid input dataLength: " + encoded.length);
            }
        }
