...
/// ESP-NOW broadcast discovery cluster: the NAN fallback bearer for devices
/// that cannot transmit NAN beacons. Reuses NAN's selection algorithm and
...
pub fn is_direct_dmesh_ssid(frame: &[u8]) -> bool {
    ...
    while offset + 2 <= frame.len() {
        ...
        // `DIRECT-dmesh` is the fixed AP name shared by Android P2P, Linux,
        // and ESP. Retain recognition of legacy MAC-derived firmware APs
        // during the rollout so they remain usable as timing/action sources.
        if id == 0
            && (frame[start..end].eq_ignore_ascii_case(b"DIRECT-dmesh")
                || frame[start..end].starts_with(b"DIRECT-DMESH-"))
        {
            return true;
        }
        offset = end;
