...
use quic_lite::{ConnectionId, ConnectionLimits, PathId, TransportPacket};

use crate::{
    probe::{ProbeRun, ProbeSender, ProbeServicePlan},
    protocol::{GetRequest, REQUEST_MAX, decode_flash_handler_request, encode_get_request},
    ...
    ConnectionDispatcher<HISTORY, PACKET, ASSOCIATIONS>
{
    ...
    pub fn receive(
        ...
        let ingress = match ingress {
            Ok(ingress) => ingress,
            Err(Error::WrongConnectionId) if self.stateless_reset_key.is_some() => {
                let Some(used) = self
                    .stateless_reset_key
