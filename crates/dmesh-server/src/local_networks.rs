...
fn network(d: &mut Decoder<'_>) -> Option<LocalNetwork> {
    let (major, count) = d.head()?;
    if major != 5 || count == u64::MAX { return None; }
...
pub fn decode_snapshot(input: &[u8]) -> Option<LocalNetworks> {
    ...
    for _ in 0..count {
        ...
        if key != "networks" { d.skip()?; continue; }
        let (array, rows) = d.head()?;
        if array != 4 || rows as usize > MAX_NETWORKS { return None; }
        for _ in 0..rows { result.networks.push(network(&mut d)?); }
    }
...
mod tests {
    ...
    fn decodes_bounded_snapshot() {
        ...
        e.map(1).unwrap(); e.text_value(b"networks").unwrap(); e.array(1).unwrap();
        e.map(11).unwrap();
        for (key, value) in [(b"interface".as_slice(), b"wlan0".as_slice())] {
            e.text_value(key).unwrap(); e.text_value(value).unwrap();
        }
        for key in [b"up".as_slice(), b"multicast", b"active", b"internet", b"validated", b"metered"] {
            e.text_value(key).unwrap(); e.boolean(true).unwrap();
        }
        for key in [b"addresses".as_slice(), b"dns_servers", b"gateways", b"transports"] {
            e.text_value(key).unwrap(); e.array(0).unwrap();
        }
        let used = e.len();
        drop(e);
